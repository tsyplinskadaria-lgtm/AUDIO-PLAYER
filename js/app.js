(() => {
    ...
    const section = document.querySelector(".slider__reading");
    const observer = new IntersectionObserver((entries => {
        entries.forEach((entry => {
            if (!entry.isIntersecting) return;
            if (introPlayed) return;
            introPlayed = true;
            const swiper = section?.swiper;
            if (!swiper) return;
            const nextSlide = swiper.slides[swiper.activeIndex + 1];
            if (!nextSlide) return;
            nextSlide.classList.add("intro-peek");
            setTimeout((() => {
                nextSlide.classList.remove("intro-peek");
            }), 1e3);
        }));
    }), {
        threshold: .4
